...
| Storage Key | `minyoung.historyStore.v1` |
| State 필드 | 4개 |
| Actions | 0개 |
...
| 파일 크기 | 602 lines |
| Persist | No (캐시만)
| Storage Key | `minyoung.scheduler.cache.schedules.v1` |
...
| 파일 크기 | 528 lines |
| Persist | v2 |
| Storage Key | `minyoung.settings.v1` |
| State 필드 | 8개 |
| Actions | 0개 |
...
| 파일 크기 | 248 lines |
| Persist | v- (세션 캐시) |
| Storage Key | `minyoung.blockComments.v1` |
| State 필드 | 2개 |
| Actions | 0개 |
...
| 파일 크기 | 222 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.scheduler.cache.mm.v1` |
...
| 파일 크기 | 207 lines |
| Persist | v2 |
| Storage Key | `minyoung.notifications.v1` |
| State 필드 | 1개 |
| Actions | 0개 |
...
| 파일 크기 | 192 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.scheduler.cache.projects.v1` |
...
| 파일 크기 | 169 lines |
| Persist | v- (세션 캐시) |
| Storage Key | `minyoung.scheduler.view.v1` |
| State 필드 | 8개 |
| Actions | 0개 |
...
| 파일 크기 | 158 lines |
| Persist | v- (세션 캐시) |
| Storage Key | `minyoung.workspace.session.v1` |
| State 필드 | 2개 |
| Actions | 0개 |
...
| 파일 크기 | 140 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.members.cache.v1` |
| State 필드 | 6개 |
| Actions | 0개 |
...
| 파일 크기 | 137 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.scheduler.cache.holidays.v1` |
...
| Storage Key | `minyoung.databaseViewPrefs.v1` |
| State 필드 | 2개 |
| Actions | 0개 |
...
| 파일 크기 | 88 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.scheduler.mmDashboard.view.v1` |
| State 필드 | 7개 |
| Actions | 0개 |
...
| 파일 크기 | 82 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.organizations.cache.v1` |
| State 필드 | 3개 |
| Actions | 0개 |
...
| 파일 크기 | 79 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.teams.cache.v1` |
| State 필드 | 3개 |
| Actions | 0개 |
...
| 파일 크기 | 60 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.database-inline-ui-prefs.v1` |
...
| 파일 크기 | 51 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.search-filter-prefs.v1` |
...
| 파일 크기 | 47 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.scheduler.filters.v1` |
| State 필드 | 2개 |
| Actions | 0개 |
...
| 파일 크기 | 34 lines |
| Persist | v- (캐시만) |
| Storage Key | `minyoung.workspace.access.cache.v1` |
...
| 스토어 | 현재 버전 | 저장소 키 |
|--------|---------|----------|
| pageStore | PAGE_STORE_PERSIST_VERSION | `minyoung.pages.v1` |
| databaseStore | DATABASE_STORE_VERSION | `minyoung.databases.v1` |
| historyStore | HISTORY_STORE_VERSION | `minyoung.historyStore.v1` |
| settingsStore | 2 | `minyoung.settings.v1` |
| notificationStore | 2 | `minyoung.notifications.v1` |
| databaseViewPrefsStore | DATABASE_VIEW_PREFS_STORE_VERSION | `minyoung.databaseViewPrefs.v1` |
| blockCommentStore | - (캐시) | `minyoung.blockComments.v1` |
| uiStore | - (캐시) | `minyoung.ui.v1` |
| 기타 캐시 스토어 | - | `minyoung.*.cache.v1` |
| authStore | No | - |
