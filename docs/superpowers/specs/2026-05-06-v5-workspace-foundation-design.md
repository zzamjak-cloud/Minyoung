...
Minyoung 를 단일 사용자 멀티 디바이스 노트 도구(v4)에서 **단일 organization 내 다중 사용자 협업 도구**로 확장한다. 관리자(Owner / Manager)가 구성원을 등록하면, 각 구성원은 본인 개인 워크스페이스와 권한이 부여된 공유 워크스페이스를 함께 사용할 수 있다. 워크스페이스 단위로 편집(edit) / 읽기(view) 권한을 분리해 인사규정처럼 일부만 편집 가능한 공간도 지원한다.
