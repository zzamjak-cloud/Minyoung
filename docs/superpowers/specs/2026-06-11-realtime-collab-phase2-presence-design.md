...
## 12. 배포 / 롤아웃
- **인프라 변경**: `infra/lambda/realtime/protocol.ts`·`sync.ts`만 수정 → `DevMinyoungRealtimeCollabStack` 재배포(테이블·라우트 변경 없음, Lambda 코드만). **develop에서 구현·dev 검증 후 사용자 명시 승인 시 배포**.
