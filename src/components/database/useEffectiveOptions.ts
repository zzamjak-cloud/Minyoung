...
// linkedScope (organization/team) 가 설정된 컬럼은 해당 store 의 데이터를 옵션화한다.
...
export function useEffectiveOptions(column: ColumnDef): SelectOption[] {
  ...
  return useMemo(
    () => effectiveOptions(column, databases, { organizations, teams }),
    [column, databases, organizations, teams],
  );
}
