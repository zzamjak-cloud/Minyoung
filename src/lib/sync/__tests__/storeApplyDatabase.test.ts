...
describe("applyRemoteDatabaseToStore", () => {
  ...
  it("DB 표시설정(viewConfigs)은 일반 LWW 로 동기화된다", () => {
    ...
    const bundle = useDatabaseStore.getState().databases["db-2"];
  ...
  it("배치(batch) 경로도 remote panelState(표시설정)를 보존한다", () => {
    // 전체 페치/새로고침은 batch 경로를 쓴다. 과거 이 경로가 panelState 를 누락해
    // 새로고침 시 표시설정이 사라졌다.
