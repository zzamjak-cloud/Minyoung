...
로컬 캐시 키는 `minyoung.database-row-index.cache.${encodeURIComponent(indexKey)}.v1` 형식이다. `indexKey`는 `resolveDatabaseRowRemoteKey(databaseId, currentWorkspaceId)` 결과이며, 보호 DB는 canonical DB ID를 사용한다.
