...
## 역할
Minyoung 에디터에 등록되는 커스텀 TipTap extension 모음. 블록 노드, 인라인 노드, 키보드 동작, 데코레이션 등을 포함한다.
...
| 파일 | Export | 설명 |
|------|--------|------|
| `imageBlock.tsx` | `ImageBlock` | 이미지 블록. width/height/align/caption 속성. `minyoung-image://` 비동기 해석 |
...
| `callout.ts` | `Callout` | 콜아웃 블록. `preset` + `emoji`(사용자 오버라이드, `data-emoji-override`) 속성. 에디터 렌더는 일반 DOM, 아이콘 클릭은 `minyoung:open-callout-icon-picker` → Editor 레벨 `IconPickerPanel` 로 처리. `updateCalloutPreset` / `updateCalloutEmoji` 커맨드 제공 |
...
| `imageInlineIcon.tsx` | `ImageInlineIcon` | 인라인 커스텀 이미지 아이콘(`minyoung-image://` 등). **슬래시 `/이모지` 커스텀 탭 전용** — `image` 블록으로 삽입하면 원본 크기로 붙는 회귀 |
