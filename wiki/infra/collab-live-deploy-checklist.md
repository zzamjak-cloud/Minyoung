...
- **반드시 라이브 `MinyoungSyncStack`(스키마 + `upsertPage` 리졸버 §9.1 스탬프)을 프론트 배포보다 먼저 배포**한다.
...
배포: `cd infra && npx cdk deploy MinyoungSyncStack` (live = DEPLOY_ENV 미지정). diff 에서 `AWS::AppSync::GraphQLSchema ... may be replaced` 는 스키마 in-place 갱신(데이터 영향 없음).

---

## 1.5 ⛔ 룸 epoch — 협업을 껐다 다시 켤 때 반드시 올린다 (2026-06-11 감사)

룸 Y 상태는 서버(`{env}minyoung-rt-ydoc`/`-ydoc-updates`, TTL 없음)와 각 브라우저
...
협업을 라이브에서 켜려면 `MinyoungRealtimeCollabStack`(WS API + 연결/YDoc 테이블 + Lambda)을 **live 에 배포**해야 한다. 현재 라이브엔 없다(Dev 만 존재).

- `cd infra && npx cdk deploy MinyoungRealtimeCollabStack`.
...
2) 사용자 명시 승인
3) (live) cd infra && npx cdk deploy MinyoungSyncStack         # §9.1 스키마 선행
4) (협업 켤 때만) npx cdk deploy MinyoungRealtimeCollabStack
